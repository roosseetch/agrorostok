...
class BasketMiddleware(object):
    ...
    def get_basket(self, request):
        ...
        cookie_basket = self.get_cookie_basket(
            settings.ISTORE_BASKET_COOKIE_OPEN, request, manager)
        ...
        if hasattr(request, 'user') and request.user.is_authenticated():
            ...
            if cookie_basket:
                ...
                request.cookies_to_delete.append(
                    settings.ISTORE_BASKET_COOKIE_OPEN)
        elif cookie_basket:
    ...
    def process_response(self, request, response):
        ...
        '''
        My own monkey patching request.basket.id
        '''
        ...
        if (hasattr(request, 'basket') and request.basket.id > 0
            and not request.user.is_authenticated()
                and settings.ISTORE_BASKET_COOKIE_OPEN not in request.COOKIES):
            cookie = "%s_%s" % (
                request.basket.id, self.get_basket_hash(request.basket.id))
            response.set_cookie(settings.ISTORE_BASKET_COOKIE_OPEN,
                                cookie,
                                max_age=settings.ISTORE_BASKET_COOKIE_LIFETIME,
                                httponly=True)
        return response
    ...
    def get_basket_hash(self, basket_id):
        ...
        return zlib.crc32(bytes(str(basket_id) + settings.SECRET_KEY, 'UTF-8'))
        '''
        Attantion!!!
        Next string is "INCORRECT" couse take too much physical memory
        For example:
        >>> sys.getsizeof(bytes(1234567890))
        1234567923
        so 1234567923 bytes are wasted
        '''
        # return bytes(zlib.crc32(bytes(str(basket_id), 'UTF-8') + bytes(settings.SECRET_KEY, 'UTF-8')))
