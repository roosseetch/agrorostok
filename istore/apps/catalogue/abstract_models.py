...
class AbstractProductAttribute(models.Model):
    ...
    def _validate_text(self, value):
        if not (type(value) == str or type(value) == bytes):
            raise ValidationError(_("Must be str or bytes"))
...
class MissingProductImage(object):
    ...
    def __init__(self, name=None):
        self.name = name if name else settings.ISTORE_MISSING_IMAGE_URL
...
class AbstractProductImage(models.Model):
    ...
    original = models.ImageField(
        _("Original"), upload_to=settings.ISTORE_IMAGE_FOLDER)
