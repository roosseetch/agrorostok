...
from istore.apps.catalogue.reviews.managers import ApprovedReviewsManagerfrom istore.core.compat import AUTH_USER_MODEL
...
class AbstractProductReview(models.Model):
    ...
    * Signed in users can always submit reviews, anonymous users can only
      submit reviews if a setting ISTORE_ALLOW_ANON_REVIEWS is set to true - it
    ...
    * By default, reviews must be approved before they are live.
      However, if a setting ISTORE_MODERATE_REVIEWS is set to false,
    ...
    * A user can only vote once on each product once
    ...
    default_status = FOR_MODERATION if settings.ISTORE_MODERATE_REVIEWS else APPROVED
