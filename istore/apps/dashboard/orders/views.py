...
from istore.apps.dashboard.orders import forms
from istore.views.generic import BulkEditMixin
from istore.apps.dashboard.reports.csv_utils import CsvUnicodeWriter
# from istore.apps.payment.exceptions import PaymentError
from istore.apps.order.exceptions import InvalidShippingEvent, InvalidStatus

# add exception from not existing payment application
...
class OrderListView(ListView, BulkEditMixin):
    ...
    desc_template = _("%(main_filter)s %(name_filter)s %(title_filter)s"
                      ...
                      "%(voucher_filter)s %(status_filter)s")
    paginate_by = 25
    description = ''
    ...
    def get_queryset(self):
        ...
        if data['name']:
            ...
            allow_anon = getattr(settings, 'ISTORE_ALLOW_ANON_CHECKOUT', False)
