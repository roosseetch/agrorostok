...
from istore.apps.shipping.methods import Free
from istore.apps.order.exceptions import UnableToPlaceOrder
...
class OrderCreator(object):
    ...
    def place_order(self, basket, total_incl_tax=None, total_excl_tax=None,
        ...
        if not status and hasattr(settings, 'ISTORE_INITIAL_ORDER_STATUS'):
            status = getattr(settings, 'ISTORE_INITIAL_ORDER_STATUS')
        try:
    ...
    def create_line_models(self, order, basket_line, extra_line_fields=None):
        ...
        if hasattr(settings, 'ISTORE_INITIAL_LINE_STATUS'):
            if not (extra_line_fields and 'status' in extra_line_fields):
                extra_line_fields['status'] = getattr(settings, 'ISTORE_INITIAL_LINE_STATUS')
