...
def get_classes(module_label, classnames):
    ...
    if app_module_path.split('.')[0] == 'istore':
        # Using core istore class
        module_path = 'istore.apps.%s' % module_label
    ...
    istore_app = "istore.apps.%s" % module_label
    imported_istore_module = __import__(istore_app, fromlist=classnames)

    return _pluck_classes([imported_local_module, imported_istore_module],
                          classnames)
...
def import_module(module_label, classes, namespace=None):
    ...
    if namespace:
        ...
        module = new_module("istore.apps.%s" % module_label)
        for classname, klass in zip(classes, klasses):
...
def feature_hidden(feature_name):
    ...
    return (feature_name is not None and
            feature_name in settings.ISTORE_HIDDEN_FEATURES)
